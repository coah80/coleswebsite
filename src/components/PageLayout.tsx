    ...
    <div className="h-screen bg-background flex flex-col overflow-y-auto lg:overflow-hidden">
        ...
        >
          <div className="flex">
            {displayTitle.split('').map((char, i) => (
              <span
                key={i}
                className="letter inline-block text-[18vw] font-black lowercase leading-none tracking-tight text-foreground"
      ...
      <header className="relative z-20 border-b border-border/20 bg-background/80 backdrop-blur-sm flex-shrink-0">
        <div className="w-full px-4 md:px-6 lg:px-8">
          <div className="flex items-center gap-6 md:gap-8 h-14 md:h-16">
            {/* Page title */}
            <h1 
              ref={headerTitleRef}
              className="text-xl md:text-2xl font-black lowercase tracking-tight text-foreground"
            >
              {title}
            </h1>
            ...
            <nav className="flex items-center gap-1">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`px-2 md:px-3 py-1.5 text-xs md:text-sm font-mono lowercase rounded-lg transition-all duration-200 ${
                    currentTab === tab.id
      ...
      <main 
        ref={contentRef}
        className="relative z-10 flex-1 w-full px-4 md:px-6 lg:px-8 py-4 md:py-5 lg:overflow-hidden"
      >
        {children}
      </main>
    </div>
